...
var ACTIONS = axboot.actionExtend(fnObj, {
    PAGE_SEARCH: function (caller, act, data) {
        axboot.ajax({
            type: "GET",
            //url: ["samples", "parent"],
            url: "/api/v1//mng/error/sh01001240",
    ...
    initView: function () {
        ...
        this.target = axboot.gridBuilder({
            ...
            columns: [
                ...
                {key: 'handleSeqNo', label: '후처리일련번호', width: 100, align: 'left', editor: 'text'},
                {key: 'handleDatetime', label: '후처리조치일시', width: 100, align: 'left', editor: 'text'},
                {key: 'handleEmpName', label: '후처리조치자명', width: 100, align: 'left', editor: 'text'},
                {key: 'handleEmpTelno', label: '후처리조치자연락처', width: 150, align: 'left', editor: 'text'},
                {key: 'handleDesc', label: '후처리조치내용', width: 100, align: 'left', editor: 'text'},
                {key: 'handleStatus', label: '조치상태', width: 100, align: 'left', editor: 'text'}
            ],
            body: {
                onClick: function () {
    ...
    getData: function (_type) {
        ...
        if (_type == "modified" || _type == "deleted") {
            list = ax5.util.filter(_list, function () {
                return this.branchCode && this.cornerCode && this.terminalNo && this.handleSeqNo && this.handleDatetime;
            });
        } else {
            list = _list;
        }
