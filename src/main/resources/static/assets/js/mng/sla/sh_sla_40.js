...
var ACTIONS = axboot.actionExtend(fnObj, {
    ...
    EXCEL_DOWNLOAD: function (caller, act, data) {
        ...
        console.log(params);
        window.location = CONTEXT_PATH + "/api/v1//mng/sla/sh_sla_40/download?" + params;
        return false;
    },
    PAGE_SAVE: function (caller, act, data) {
        ...
        if (caller.gridView01.validate()) {
            ...
            if (data === "save") {
                saveUrl = "/api/v1//mng/sla/sh_sla_40";
            } else {
                saveUrl = "/api/v1//mng/sla/sh_sla_40/writeSla";
            }
...
fnObj.gridView01 = axboot.viewExtend(axboot.gridView, {
    ...
    initView: function () {
        ...
        this.target = axboot.gridBuilder({
            ...
            columns: [
                {key: 'seqNo', label: '일련번호', width: 100, align: 'center'},
                {key: 'corpCode', label: '업체코드', width: 70, align: 'center'},
                {key: 'gubun', label: '구분', width: 70, align: 'center'},
                {
                ...
                {key: 'calleeReqDatetime', label: '해피콜 실시일시', width: 130, align: 'center'},
                {
                    key: 'checkpointScore1', label: '충분설명', width: 70, align: 'center',
                ...
                {
                    key: 'checkpointScore2', label: '인사실시', width: 70, align: 'center',
                    formatter: function formatter() {
                        var checkpointScore = "";
                    ...
                    key: 'checkpointScore3', label: '친절어투', width: 70, align: 'center',
                    formatter: function formatter() {
                        var checkpointScore = "";
                    ...
                    key: 'checkpointScore4', label: '사과실시', width: 70, align: 'center',
                    formatter: function formatter() {
                        var checkpointScore = "";
                ...
                {key: 'happycallAgent', label: '해피콜실시자', width: 90, align: 'center'},
                {key: 'happycallDatetime', label: '해피콜실시일시', width: 130, align: 'center'},
                {key: 'errorDatetime', label: '장애발생일시', width: 130, align: 'center'},
                {key: 'calleeReqUser', label: '출동요청자', width: 80, align: 'center'},
                {key: 'customer', label: '고객명', width: 80, align: 'center'},
                {key: 'telNo', label: '전화번호', width: 100, align: 'center'},
                {key: 'content', label: '고객의견', width: 250, align: 'left'}
            ],
            body: {
                onClick: function () {
