...
GridWrapper.prototype.setColumnInfo = function(list) {
    ...
    var generateColumnNFieldInfo = function(data, obj, fieldObj)
    {
        obj = {
            name : data.name,
            fieldName : data.name,
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        columnNFieldInfo = generateColumnNFieldInfo(data,obj,fieldObj);
        if(!data.columnList && data.columnList.length > 0)
        {
            innerColumnInfo = new Array();
            for(var j = 0; j < data.columnList.length; j++)
            {
                innerColumnInfo.push()
            }
        }

        columnList.push(columnNFieldInfo.obj);
        fieldList.push($.extend(columnNFieldInfo.fieldObj,{
            fieldName : data.name,
