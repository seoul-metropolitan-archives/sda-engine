...
axboot.pageAutoHeight = {
    ...
    align: function align() {
        ...
        (function () {
            ...
            //console.log('page-header :: ' + $('form').attr('page-header'));
    ...
    Tooltip.prototype.tip = function () {
        if (!this.$tip) {
            ...
            if (this.$tip.length != 1) {
                throw new Error(this.type + ' `tpls` option must consist of exactly 1 top-level element!');
            }
        }
        return this.$tip;
    };
 ...
 *        /// --> 이것들을 list로 담아서  [PUT] "/api/v1/menu/auth"
 *    ],
 *    body: {
 *        onClick: function () {
...
axboot.modal = function () {
    ...
    var open2 = function open(modalConfig) {
        var width = 0;
        if ("COMMON_POPUP2" == modalConfig.modalType && modalConfig["sendData"]) {
            var list = null;
            var reqData = modalConfig.sendData();
            var popupName = "";
            axboot.ajax({
...
axboot.commonCodeFilter = function (categoryCode) {
    ...
    commonCodeArray.forEach(function (item, field) {
        ...
        defaultYNArr.push(item['defaultYN'])
        var attrObj = {};
    ...
    return $.extend({}, {nameArr: nameArr}, {codeArr: codeArr}, {attrArr: attrArr}, {cCodeArr: cCodeArr},{defaultYNArr:defaultYNArr});
}
/**
 * 공통코드를 Grid에서 사용하기 위해 label과 value를 분리해주는 함수
 * @param array
    ...
    */
    ...
    if(parent.window.fnObj.tabView)
    {
        parent.window.fnObj.tabView.list.forEach(function (_item, idx) {
            if (_item.status == "on" && _item.menuParams != null) {
                if( _item.menuParams.type != undefined)
